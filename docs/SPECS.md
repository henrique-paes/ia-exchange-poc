# Specs Index (SDD)

Specifications are written **before** tests and implementation. Each card in
`TASKS.md` references one of these documents.

| Spec | File | Status |
|------|------|--------|
| User domain | [`specs/user.md`](./specs/user.md) | REVIEWED |
| Book domain | [`specs/book.md`](./specs/book.md) | REVIEWED |
| Rental domain | [`specs/rental.md`](./specs/rental.md) | REVIEWED |
| API contract | [`specs/api.md`](./specs/api.md) | REVIEWED |
| UI styling standard | [`specs/ui-style.md`](./specs/ui-style.md) | REVIEWED |
| UI components | [`specs/ui-components.md`](./specs/ui-components.md) | REVIEWED |

> Update the **Status** column as specs are written and reviewed (TODO → DRAFT → REVIEWED).
